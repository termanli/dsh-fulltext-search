 ...
 *
 *   POST /fts/api/search  { sessionId, cwd?, query, isRegex?, caseSensitive?,
 ...
 * never escapes the session cwd — there is no caller-targetable path input.
 *
 ...
 * dsh-better-sidebar's own route hardening.
 */
...
function requireAbsolute(raw) {
  if (!raw.startsWith('/') && !/^[A-Za-z]:[\\/]/.test(raw) && !raw.startsWith('\\\\')) {
    throw new ApiError('bad-request', `expected an absolute path, got "${raw}"`)
  }
  return raw
}

// ── session cwd resolution (mirror of better-sidebar's sessionCwdOf) ────────
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (method === 'search') {
              ...
              const cwd = sessionCwdOf(ctx.sessions, sessionId, clientCwd)
