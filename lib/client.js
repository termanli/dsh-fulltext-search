...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * with `err.code` set (e.g. `invalid-regex`, `session-not-found`).
		 *
		 * Security: NO `cwd` is sent. The host resolves the search root from
		 * its server-side session record for `sessionId` only; a client-supplied
		 * cwd would let a forged sessionId widen the search to any readable
		 * host directory, so the field was removed from the protocol entirely.
		 */
		...
		function SearchTab(props) {
			...
			const run = () => {
				...
				searchApi(scope, q, { caseSensitive, wholeWord, isRegex, smartCase }, controller.signal)
					...
					.catch((err) => {
						...
						const friendly = {
							"session-not-found": "当前会话不存在或已失效，请刷新页面后重试。",
							"no-session-cwd": "当前会话没有工作目录，无法搜索。",
						};
						setError(err && err.code && friendly[err.code]
							? friendly[err.code]
			...
			const openFile = (abs) => {
				try {
					// Contract boundary: `openFile` is an optional capability of
					// the better-sidebar service; feature-detect it and fall
					// back to the tab host's callback when unavailable.
		 ...
		 *
		 * Dependency boundary: this plugin requires the dsh-better-sidebar
		 * service contract (`ctx.betterSidebar.registerTab`). The registry
		 * cannot declare an installable dependency, so the requirement is
		 * enforced here by feature detection: install dsh-better-sidebar
		 * FIRST, then this plugin; on upgrade, upgrade dsh-better-sidebar
		 * first. When the service is missing we log a clear warning and skip
		 * registration instead of crashing the GUI. */
		function apply(ctx) {
			const registry = ctx && ctx.betterSidebar;
			if (!registry || typeof registry.registerTab !== "function") {
				console.warn(
					"[dsh-fulltext-search] 未检测到 dsh-better-sidebar 的 registerTab 服务，全文搜索 tab 未注册。" +
					"请先安装/升级 dsh-better-sidebar，再重启 DSH 并硬刷新浏览器（Ctrl/Cmd+Shift+R）。",
				);
				return;
			}
			ctx.effect(() =>
